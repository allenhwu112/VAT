# vat-database-initialization Specification

## Purpose

This capability provides a repeatable and auditable way to initialize an existing VAT database with the complete application schema, database procedures, and shared connection configuration contract.

## Requirements

### Requirement: The system uses the shared VAT connection configuration

The API and schema migration command SHALL resolve the target SQL Server connection from `ConnectionStrings:VAT`, and a `ConnectionStrings__VAT` environment value SHALL override the configured default. The current default SHALL target the existing `VAT` database on `192.168.25.20`; deployments that should not store credentials in tracked files SHALL use the environment override or a secret manager.
